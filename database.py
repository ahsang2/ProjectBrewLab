...
def upsert_df_to_postgres(df: pd.DataFrame, table_name: str, schema: str | None = None, pk_col: str = "Date"):
    ...
    # Map the provided primary key column name (as in original dataframe) to the normalized column name
    normalized_pk_col = mapper.get(pk_col)
    if not normalized_pk_col:
        raise ValueError(f"Primary key '{pk_col}' not found in original DataFrame columns.")

    # Ensure the date column exists and is in date format
    df = _ensure_date_column(df, normalized_pk_col)
